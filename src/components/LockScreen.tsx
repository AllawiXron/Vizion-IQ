...
export default function LockScreen({ onSuccess }: LockScreenProps) {
  ...
  // Load valid codes from localStorage, or initialize defaults if empty
  const getValidCodes = (): string[] => {
    ...
    if (stored) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Extract active (non-revoked) codes
          return parsed
    ...
    const defaultCodes = [
      { code: "iraq#gold", isRevoked: false, buyerName: "مشتري متميز", dateAdded: new Date().toLocaleDateString("ar-IQ") },
      { code: "premium2026", isRevoked: false, buyerName: "أكاديمية التسويق", dateAdded: new Date().toLocaleDateString("ar-IQ") },
      { code: "admin#1", isRevoked: false, buyerName: "المدير العام", dateAdded: new Date().toLocaleDateString("ar-IQ") },
      { code: "ali#1", isRevoked: false, buyerName: "علي الرافدين", dateAdded: new Date().toLocaleDateString("ar-IQ") }
    ];
    localStorage.setItem("sales_guide_codes", JSON.stringify(defaultCodes));
    return defaultCodes.map(c => c.code);
  };
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col justify-center items-center overflow-hidden font-sans select-none px-4 py-8">
        ...
        <div className="glass-panel-gold rounded-2xl p-8 md:p-10 shadow-2xl relative overflow-hidden backdrop-blur-xl border border-white/10">
          ...
          <div className="flex flex-col items-center mb-8 text-center">
            ...
            <h1 className="text-3xl font-extrabold text-white tracking-tight mb-2">
              دليل المبيعات العراقي
            </h1>
            <p className="text-[#F0F4FF]/75 text-sm md:text-base font-light">
              محتوى حصري للأعضاء والمسوقين المسجلين
            </p>
      ...
      <div className="absolute bottom-4 text-center z-10 text-[11px] text-[#F0F4FF]/30 tracking-wider">
        © 2026 دليل المبيعات العراقي. جميع الحقوق محفوظة للنخبة المسجلة.
      </div>
    </div>
  );
}
